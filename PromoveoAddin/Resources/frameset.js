...
//The string ##FOLDERNAME## Must be replaced porgrammatically by  filename_files
var xmlData = XMLData("##FOLDERNAME##/data.xml");
var xmlPermissionData = XMLData("##FOLDERNAME##/userdata.xml");
...
{
    //Added by BLEL/18.03.2013
    //The parameter "Username" is set by the Promoveo Sharepoint Webpart
    var username = getURLParameter("Username");
    var PageName = g_FileList[index].PageName;

    if (IsUserOfModel(PageName, username))
    {
        if (viewMgr)
        {
    ...
    {
        window.alert("Sorry, you don't have permissions to access this page.");
    }
}
...
//--------------------------------------------------------------------------------------------------------------------------------------
//CUSTOM FUNCTIONS FOR PROMOVEO USER MANAGEMENT
//Added by BLEL/18.03.2013
//--------------------------------------------------------------------------------------------------------------------------------------
function IsUserOfModel(Modelname, Username) {
    //The function expects that shortnames are in upper case (4lc)
    Username = Username.toUpperCase();
    if (xmlPermissionData != null) {
        var modelQueryString = './/SerializableModelUser[Model = "' + Modelname + '"]/Users/SerializableUser[Shortname="' + Username + '"]';
        var modelObj = SelectSingleNode(xmlPermissionData, modelQueryString);
        if (modelObj != null) {
            return true;
        }
        else {
